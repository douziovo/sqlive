...
import type { ExecuteResponse, ExecuteRequest } from '../model/ApiTypes';
...
export function useSqlEngine() {
    ...
    const {
        ...
        switchTab, addTab, closeTab, setTabDbName,
        updateCode, markClean, importFile, exportTab, exportAllTabs,
    ...
    const { getLastValidCode, updateRow, deleteRow, insertRowUI, addNewTable, dropTableUI } = useBidirectionalSync(code, db, mode, flashCode, transition);
    ...
    const { shouldReset, submitNow, deleteDb, dbList } = useDatabaseLifecycle(activeTab, tabs, setTabDbName, mode, executeSqlRemote, transition);
    ...
    return { db, code, highlight, highlightedCodeChunk, executionError, isLoading, updateRow, deleteRow, addNewTable, dropTableUI, insertRowUI, tabs, activeTabId, switchTab, addTab, closeTab, setTabDbName, dbList, submitNow, deleteDb, importFile, exportTab, exportAllTabs, markClean };
}
