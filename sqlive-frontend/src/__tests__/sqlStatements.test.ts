...
import { mockSuccess, tick, setupSqlEngine, teardownSqlEngine, type SqlEngineSetup } from './test-utils';
...
describe('SQL statement handling', () => {
    ...
    describe('DDL — ALTER, DROP, INDEX, VIEW', () => {
        ...
        it('should handle ALTER TABLE ADD COLUMN', async () => {
            mockSuccess(fetchSpy, {
                tables: [{ name: 'users', columns: ['id', 'email'], columnTypes: { id: 'INTEGER', email: 'TEXT | NOT NULL DEFAULT' }, data: [{ id: 1, email: '' }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle CREATE INDEX', async () => {
            mockSuccess(fetchSpy, {
                ...
                indexes: [{ name: 'idx_name', tableName: 'users', columns: ['name'], unique: false, sql: 'CREATE INDEX idx_name ON users (name)' }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle CREATE UNIQUE INDEX', async () => {
            mockSuccess(fetchSpy, {
                ...
                indexes: [{ name: 'idx_email', tableName: 'users', columns: ['email'], unique: true, sql: 'CREATE UNIQUE INDEX idx_email ON users (email)' }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle CREATE INDEX IF NOT EXISTS', async () => {
            mockSuccess(fetchSpy, {
                ...
                indexes: [{ name: 'idx_a', tableName: 't', columns: ['a'], unique: false, sql: 'CREATE INDEX IF NOT EXISTS idx_a ON t (a)' }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle CREATE VIEW', async () => {
            mockSuccess(fetchSpy, {
                ...
                views: [{ name: 'high_earners', sql: 'CREATE VIEW high_earners AS SELECT name, salary FROM employees WHERE salary > 5000' }],
            });
            const engine = useSqlEngine();
...
SELECT * FROM high_earners;`;
    ...
    describe('DML — INSERT', () => {
        ...
        it('should handle INSERT OR REPLACE', async () => {
            mockSuccess(fetchSpy, {
                tables: [{ name: 't', columns: ['id', 'name'], columnTypes: { id: 'INTEGER', name: 'TEXT' }, data: [{ id: 1, name: 'Alice' }] }],
            });
            const engine = useSqlEngine();
    ...
    describe('DML — UPDATE and DELETE', () => {
        it('should handle UPDATE with WHERE', async () => {
            mockSuccess(fetchSpy, {
                tables: [{ name: 't', columns: ['id', 'name'], columnTypes: { id: 'INTEGER', name: 'TEXT' }, data: [{ id: 1, name: 'Bob' }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle UPDATE with multiple columns', async () => {
            mockSuccess(fetchSpy, {
                tables: [{ name: 't', columns: ['id', 'name', 'age'], columnTypes: { id: 'INTEGER', name: 'TEXT', age: 'INTEGER' }, data: [{ id: 1, name: 'Bob', age: 30 }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle UPDATE without WHERE', async () => {
            mockSuccess(fetchSpy, {
                tables: [{ name: 't', columns: ['active'], columnTypes: { active: 'INTEGER' }, data: [{ active: 1 }, { active: 1 }] }],
            });
            const engine = useSqlEngine();
    ...
    describe('DQL — SELECT', () => {
        ...
        it('should handle SELECT DISTINCT', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name'], columnTypes: { name: 'TEXT' }, data: [{ name: 'Alice' }, { name: 'Bob' }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle SELECT with LIMIT and OFFSET', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['x'], columnTypes: { x: 'INTEGER' }, data: [{ x: 11 }, { x: 12 }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle SELECT with GROUP BY and HAVING', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['dept', 'cnt'], columnTypes: { dept: 'TEXT', cnt: 'INTEGER' }, data: [{ dept: 'IT', cnt: 10 }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle SELECT with aggregate functions', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['cnt', 'sum', 'avg', 'max', 'min'], columnTypes: { cnt: 'INTEGER', sum: 'REAL', avg: 'REAL', max: 'INTEGER', min: 'INTEGER' }, data: [{ cnt: 100, sum: 5000, avg: 50, max: 99, min: 1 }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle INNER JOIN', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['id', 'a_id'], columnTypes: { id: 'INTEGER', a_id: 'INTEGER' }, data: [{ id: 1, a_id: 1 }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle CROSS JOIN', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['id'], columnTypes: { id: 'INTEGER' }, data: [{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle table alias with AS', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name'], columnTypes: { name: 'TEXT' }, data: [{ name: 'Alice' }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle table alias without AS', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'dept'], columnTypes: { name: 'TEXT', dept: 'TEXT' }, data: [{ name: 'Alice', dept: 'IT' }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle subquery in FROM', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['id', 'name'], columnTypes: { id: 'INTEGER', name: 'TEXT' }, data: [{ id: 1, name: 'Alice' }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle CASE expression', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'level'], columnTypes: { name: 'TEXT', level: 'TEXT' }, data: [{ name: 'Alice', level: 'High' }] }],
            });
            const engine = useSqlEngine();
    ...
    describe('Parsing edge cases', () => {
        it('should handle semicolons inside single-quoted strings', async () => {
            mockSuccess(fetchSpy, { tables: [{ name: 't', columns: ['x'], columnTypes: { x: 'TEXT' }, data: [{ x: 'hello; world' }] }] });
            const engine = useSqlEngine();
        ...
        it('should handle escaped single quotes', async () => {
            mockSuccess(fetchSpy, { tables: [{ name: 't', columns: ['x'], columnTypes: { x: 'TEXT' }, data: [{ x: "it's" }] }] });
            const engine = useSqlEngine();
        ...
        it('should handle -- line comments', async () => {
            mockSuccess(fetchSpy, { tables: [{ name: 't', columns: ['x'], columnTypes: { x: 'INTEGER' }, data: [{ x: 1 }] }] });
            const engine = useSqlEngine();
...
SELECT * FROM t;`;
    ...
    describe('Multi-statement scripts', () => {
        it('should handle full DDL+DML+DQL script', async () => {
            mockSuccess(fetchSpy, {
                tables: [
                    { name: 'employees', columns: ['id', 'name', 'salary'], columnTypes: { id: 'INTEGER', name: 'TEXT', salary: 'REAL' }, data: [{ id: 1, name: 'Alice', salary: 7500 }] },
                    { name: 'departments', columns: ['id', 'name'], columnTypes: { id: 'INTEGER', name: 'TEXT' }, data: [{ id: 1, name: 'IT' }] },
                ],
                queryResults: [
                    { name: '查询结果', columns: ['name', 'total'], columnTypes: { name: 'TEXT', total: 'INTEGER' }, data: [{ name: 'Alice', total: 7500 }] },
                ]
            });
            const engine = useSqlEngine();
...
`;
    ...
    describe('SQLite-specific features', () => {
        it('should handle AUTOINCREMENT', async () => {
            mockSuccess(fetchSpy, { tables: [{ name: 't', columns: ['id'], columnTypes: { id: 'INTEGER | PRIMARY KEY AUTOINCREMENT' }, data: [] }] });
            const engine = useSqlEngine();
        ...
        it('should handle CHECK constraint', async () => {
            mockSuccess(fetchSpy, { tables: [{ name: 't', columns: ['age'], columnTypes: { age: 'INTEGER | CHECK' }, data: [] }] });
            const engine = useSqlEngine();
        ...
        it('should handle DEFAULT with expressions', async () => {
            mockSuccess(fetchSpy, { tables: [{ name: 't', columns: ['created'], columnTypes: { created: 'TEXT | DEFAULT' }, data: [] }] });
            const engine = useSqlEngine();
        ...
        it('should handle UNIQUE constraint', async () => {
            mockSuccess(fetchSpy, { tables: [{ name: 't', columns: ['email'], columnTypes: { email: 'TEXT | UNIQUE' }, data: [] }] });
            const engine = useSqlEngine();
        ...
        it('should handle sqlite_master query', async () => {
            mockSuccess(fetchSpy, { tables: [], queryResults: [{ name: '查询结果', columns: ['name'], columnTypes: { name: 'TEXT' }, data: [{ name: 't' }] }] });
            const engine = useSqlEngine();
        ...
        it('should handle date/time functions', async () => {
            mockSuccess(fetchSpy, {
                tables: [{ name: 'events', columns: ['id', 'created_at', 'event_date', 'event_time'], columnTypes: { id: 'INTEGER', created_at: 'TEXT', event_date: 'DATE', event_time: 'TIME' }, data: [] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle string functions', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['upper', 'lower', 'len', 'substr', 'replace', 'trim', 'full_title'], columnTypes: { upper: 'TEXT', lower: 'TEXT', len: 'INTEGER', substr: 'TEXT', replace: 'TEXT', trim: 'TEXT', full_title: 'TEXT' }, data: [] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle math functions', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['abs', 'round', 'random', 'max', 'min'], columnTypes: { abs: 'INTEGER', round: 'REAL', random: 'INTEGER', max: 'INTEGER', min: 'INTEGER' }, data: [{ abs: 42, round: 3.14, random: 123, max: 10, min: 1 }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle trigger-like CONFLICT clause', async () => {
            mockSuccess(fetchSpy, {
                tables: [{ name: 't', columns: ['id', 'name'], columnTypes: { id: 'INTEGER | PRIMARY KEY', name: 'TEXT' }, data: [] }],
            });
            const engine = useSqlEngine();
            engine.code.value = `CREATE TABLE t (
        ...
        it('should handle WITH clause (CTE)', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'salary', 'avg_sal'], columnTypes: { name: 'TEXT', salary: 'REAL', avg_sal: 'REAL' }, data: [{ name: 'Alice', salary: 9000, avg_sal: 7000 }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle window functions (SQLite 3.25+)', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'salary', 'rank', 'dept_rank'], columnTypes: { name: 'TEXT', salary: 'REAL', rank: 'INTEGER', dept_rank: 'INTEGER' }, data: [{ name: 'Alice', salary: 9000, rank: 1, dept_rank: 1 }] }],
            });
            const engine = useSqlEngine();
            engine.code.value = `SELECT
                name,
    ...
    describe('Triggers', () => {
        it('should handle CREATE TRIGGER with BEGIN...END', async () => {
            mockSuccess(fetchSpy, {
                ...
                triggers: [{ name: 'trg_test', tableName: 'log', sql: 'CREATE TRIGGER trg_test AFTER INSERT ON log BEGIN INSERT INTO log (msg) VALUES (\'trigger fired\'); END' }],
            });
            const engine = useSqlEngine();
...
END;`;
        ...
        it('should handle multiple triggers on same table', async () => {
            mockSuccess(fetchSpy, {
                tables: [{ name: 't', columns: ['x'], columnTypes: { x: 'INTEGER' }, data: [] }, { name: 'audit', columns: ['msg'], columnTypes: { msg: 'TEXT' }, data: [] }],
                triggers: [
...
WHERE e.salary > d.avg_sal;`;
        ...
        it('should handle recursive CTE (number sequence)', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['n', 'square'], columnTypes: { n: 'INTEGER', square: 'INTEGER' }, data: Array.from({ length: 10 }, (_, i) => ({ n: i + 1, square: (i + 1) ** 2 })) }],
            });
            const engine = useSqlEngine();
...
SELECT n, n * n AS square FROM seq;`;
        ...
        it('should handle recursive CTE (Fibonacci)', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['n', 'fib_value'], columnTypes: { n: 'INTEGER', fib_value: 'INTEGER' }, data: Array.from({ length: 11 }, (_, i) => ({ n: i, fib_value: i })) }],
            });
            const engine = useSqlEngine();
...
SELECT n, a AS fib_value FROM fib;`;
        ...
        it('should handle recursive CTE (tree traversal)', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['id', 'name', 'path', 'level'], columnTypes: { id: 'INTEGER', name: 'TEXT', path: 'TEXT', level: 'INTEGER' }, data: [] }],
            });
            const engine = useSqlEngine();
...
ON CONFLICT DO NOTHING;`;
    ...
    describe('NULL handling', () => {
        it('should handle COALESCE', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['val'], columnTypes: { val: 'TEXT' }, data: [{ val: 'fallback' }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle NULLIF', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['same', 'diff'], columnTypes: { same: 'INTEGER', diff: 'INTEGER' }, data: [{ same: null, diff: 10 }] }],
            });
            const engine = useSqlEngine();
    ...
    describe('Additional JOIN types', () => {
        it('should handle SELF JOIN', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'colleague'], columnTypes: { name: 'TEXT', colleague: 'TEXT' }, data: [{ name: 'Alice', colleague: 'Bob' }] }],
            });
            const engine = useSqlEngine();
...
WHERE e1.id != e2.id;`;
    ...
    describe('All window function variants', () => {
        it('should handle ROW_NUMBER, RANK, DENSE_RANK, NTILE', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'salary', 'rn', 'rk', 'dr', 'nt'], columnTypes: { name: 'TEXT', salary: 'REAL', rn: 'INTEGER', rk: 'INTEGER', dr: 'INTEGER', nt: 'INTEGER' }, data: [{ name: 'Alice', salary: 9000, rn: 1, rk: 1, dr: 1, nt: 1 }] }],
            });
            const engine = useSqlEngine();
...
FROM employees;`;
        ...
        it('should handle LAG and LEAD', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'salary', 'prev', 'next'], columnTypes: { name: 'TEXT', salary: 'REAL', prev: 'REAL', next: 'REAL' }, data: [{ name: 'Bob', salary: 7000, prev: 5000, next: 9000 }] }],
            });
            const engine = useSqlEngine();
            engine.code.value = `SELECT name, salary,
...
FROM employees;`;
        ...
        it('should handle FIRST_VALUE and LAST_VALUE', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'salary', 'highest', 'lowest'], columnTypes: { name: 'TEXT', salary: 'REAL', highest: 'TEXT', lowest: 'TEXT' }, data: [{ name: 'Alice', salary: 9000, highest: 'Alice', lowest: 'Bob' }] }],
            });
            const engine = useSqlEngine();
            engine.code.value = `SELECT name, salary,
...
FROM employees;`;
        ...
        it('should handle CUME_DIST and PERCENT_RANK', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'salary', 'cd', 'pr'], columnTypes: { name: 'TEXT', salary: 'REAL', cd: 'REAL', pr: 'REAL' }, data: [{ name: 'Alice', salary: 9000, cd: 1.0, pr: 1.0 }] }],
            });
            const engine = useSqlEngine();
            engine.code.value = `SELECT name, salary,
...
FROM employees;`;
    ...
    describe('String functions', () => {
        it('should handle basic string functions', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['u', 'l', 'len', 'sub', 'rep', 'tr', 'ltr', 'rtr', 'pos', 'concat'], columnTypes: { u: 'TEXT', l: 'TEXT', len: 'INTEGER', sub: 'TEXT', rep: 'TEXT', tr: 'TEXT', ltr: 'TEXT', rtr: 'TEXT', pos: 'INTEGER', concat: 'TEXT' }, data: [{ u: 'HELLO', l: 'world', len: 4, sub: 'hel', rep: 'a_b', tr: 'x', ltr: 'a', rtr: 'b', pos: 7, concat: 'AB' }] }],
            });
            const engine = useSqlEngine();
            engine.code.value = `SELECT
    ...
    describe('Date / time functions', () => {
        it('should handle date and time functions', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['d', 't', 'dt', 'tomorrow', 'month_start', 'fmt', 'jd', 'ue'], columnTypes: { d: 'TEXT', t: 'TEXT', dt: 'TEXT', tomorrow: 'TEXT', month_start: 'TEXT', fmt: 'TEXT', jd: 'REAL', ue: 'INTEGER' }, data: [{ d: '2025-01-15', t: '10:30:00', dt: '2025-01-15 10:30:00', tomorrow: '2025-01-16', month_start: '2025-01-01', fmt: '2025-01-15', jd: 2460000.0, ue: 1700000000 }] }],
            });
            const engine = useSqlEngine();
            engine.code.value = `SELECT
    ...
    unixepoch('now') AS ue;`;
        ...
        it('should handle date calculations', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['hire_date', 'days_employed'], columnTypes: { hire_date: 'TEXT', days_employed: 'REAL' }, data: [{ hire_date: '2020-01-01', days_employed: 2000 }] }],
            });
            const engine = useSqlEngine();
            engine.code.value = `SELECT
    hire_date,
...
FROM employees;`;
    ...
    describe('Math and aggregate extras', () => {
        it('should handle COUNT DISTINCT', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['dept_count'], columnTypes: { dept_count: 'INTEGER' }, data: [{ dept_count: 3 }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle GROUP_CONCAT with ORDER BY', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['dept_id', 'members'], columnTypes: { dept_id: 'INTEGER', members: 'TEXT' }, data: [{ dept_id: 1, members: 'Alice, Bob' }] }],
            });
            const engine = useSqlEngine();
...
FROM employees GROUP BY dept_id;`;
        ...
        it('should handle TOTAL', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['total'], columnTypes: { total: 'REAL' }, data: [{ total: 50000 }] }],
            });
            const engine = useSqlEngine();
        ...
        it('should handle ABS and ROUND', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['a', 'r', 'rand'], columnTypes: { a: 'INTEGER', r: 'REAL', rand: 'INTEGER' }, data: [{ a: 42, r: 3.142, rand: 73 }] }],
            });
            const engine = useSqlEngine();
    ...
    describe('DML extras', () => {
        it('should handle INSERT INTO ... SELECT', async () => {
            mockSuccess(fetchSpy, {
                tables: [{ name: 'backup', columns: ['name', 'salary'], columnTypes: { name: 'TEXT', salary: 'REAL' }, data: [{ name: 'Alice', salary: 12000 }] }],
            });
            const engine = useSqlEngine();
...
INSERT INTO backup SELECT name, salary FROM employees WHERE salary > 10000;`;
        ...
        it('should handle UPDATE with subquery', async () => {
            mockSuccess(fetchSpy, {
                tables: [{ name: 'employees', columns: ['id', 'salary'], columnTypes: { id: 'INTEGER', salary: 'REAL' }, data: [{ id: 1, salary: 7000 }] }],
            });
            const engine = useSqlEngine();
    ...
    describe('Subquery extras', () => {
        it('should handle scalar subquery in SELECT', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'salary', 'avg_all'], columnTypes: { name: 'TEXT', salary: 'REAL', avg_all: 'REAL' }, data: [{ name: 'Alice', salary: 9000, avg_all: 7500 }] }],
            });
            const engine = useSqlEngine();
            engine.code.value = `SELECT name, salary,
...
WHERE id NOT IN (SELECT emp_id FROM projects);`;
        ...
        it('should handle ALL / ANY subquery (using MAX as SQLite does not support ALL)', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'salary'], columnTypes: { name: 'TEXT', salary: 'REAL' }, data: [{ name: 'Alice', salary: 9000 }] }],
            });
            const engine = useSqlEngine();
...
WHERE salary > (SELECT MAX(salary) FROM employees WHERE dept_id = 2);`;
        ...
        it('should handle multi-column subquery', async () => {
            mockSuccess(fetchSpy, {
                queryResults: [{ name: '查询结果', columns: ['name', 'dept_id', 'salary'], columnTypes: { name: 'TEXT', dept_id: 'INTEGER', salary: 'REAL' }, data: [{ name: 'Alice', dept_id: 1, salary: 9000 }] }],
            });
            const engine = useSqlEngine();
