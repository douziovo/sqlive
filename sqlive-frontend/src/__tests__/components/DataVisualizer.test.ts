...
const mockIndexes: IndexInfo[] = [
    ...
    {
        name: 'idx_users_name_dept',
        tableName: 'users',
        columns: ['name', 'dept_id'],
        unique: false,
        sql: 'CREATE INDEX idx_users_name_dept ON users(name, dept_id)'
    },
    {
    ...
    },
    {
        ...
        columns: ['status'],
        unique: false,
        sql: 'CREATE INDEX idx_active_users ON users(status) WHERE status = 1'
    }
]
...
const mockTriggers: TriggerInfo[] = [
    {
        ...
        tableName: 'users',
        sql: 'CREATE TRIGGER trg_users_after_insert AFTER INSERT ON users BEGIN INSERT INTO audit_log VALUES(NEW.id); END'
    },
    {
        name: 'trg_users_before_update',
        tableName: 'users',
        sql: 'CREATE TRIGGER trg_users_before_update BEFORE UPDATE ON users BEGIN UPDATE users SET updated_at = CURRENT_TIMESTAMP; END'
    }
]
