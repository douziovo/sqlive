...
const styledEdges = computed<Edge[]>(() => {
  const baseOpacity = Math.round(lod.edgeOpacityForZoom(viewport.zoomLevel.value) * 100) / 100
  const cacheKey = `${baseOpacity}|${hover.hoveredNodeId.value || ''}`
  if (cacheKey === _edgeCacheKey.value && _edgeCache.value.length) return _edgeCache.value
  ...
  const result = props.edges.map((edge) => {
    ...
    if (isSucc) return { ...edge, style: { ...edge.style, opacity: 1, stroke: '#3b82f6', strokeWidth: 2, strokeDasharray: 'none', filter: 'drop-shadow(0 0 5px rgba(59,130,246,0.5))' } }
    if (isPred) return { ...edge, style: { ...edge.style, opacity: 1, stroke: '#f59e0b', strokeWidth: 2, strokeDasharray: 'none', filter: 'drop-shadow(0 0 5px rgba(245,158,11,0.5))' } }
