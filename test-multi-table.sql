...
INSERT INTO departments VALUES (2, '市场部', '上海', 300000);
INSERT INTO departments VALUES (3, '人事部', '广州', 150000);
INSERT INTO departments VALUES (4, '财务部', '深圳', 200000);

INSERT INTO employees VALUES (1, '张三', 28, 15000, 1, '2020-03-15', 'zhangsan@example.com');
INSERT INTO employees VALUES (2, '李四', 32, 18000, 1, '2019-07-01', 'lisi@example.com');
INSERT INTO employees VALUES (3, '王五', 26, 12000, 1, '2021-01-10', 'wangwu@example.com');
INSERT INTO employees VALUES (4, '赵六', 35, 20000, 2, '2018-05-20', 'zhaoliu@example.com');
INSERT INTO employees VALUES (5, '孙七', 29, 9500, 2, '2020-11-01', 'sunqi@example.com');
INSERT INTO employees VALUES (6, '周八', 40, 22000, 3, '2017-02-14', 'zhouba@example.com');
INSERT INTO employees VALUES (7, '吴九', 24, 8000, 3, '2022-06-30', 'wujiu@example.com');
INSERT INTO employees VALUES (8, '郑十', 31, 17000, 4, '2019-09-09', 'zhengshi@example.com');

INSERT INTO projects VALUES (1, '云平台重构', '2022-01-01', '2022-12-31', 200000);
INSERT INTO projects VALUES (2, '移动App开发', '2022-03-01', '2023-06-30', 150000);
INSERT INTO projects VALUES (3, '数据分析系统', '2022-06-01', NULL, 100000);

INSERT INTO employee_projects VALUES (1, 1, '架构师', 120);
INSERT INTO employee_projects VALUES (2, 1, '后端开发', 200);
INSERT INTO employee_projects VALUES (3, 1, '前端开发', 180);
INSERT INTO employee_projects VALUES (1, 2, '技术顾问', 60);
INSERT INTO employee_projects VALUES (4, 2, '产品经理', 100);
INSERT INTO employee_projects VALUES (5, 2, 'UI设计', 160);
INSERT INTO employee_projects VALUES (2, 3, '数据工程师', 150);
INSERT INTO employee_projects VALUES (6, 3, '数据分析师', 120);
...
VALUES (
    42, -100, 127, 32767, 8388607, 9223372036854775807,
...
-- 涨薪：技术部全员涨 10%
UPDATE employees SET salary = ROUND(salary * 1.1, 2) WHERE dept_id = 1;

-- 删除已完成项目（有 end_date 且在 2023 年之前）
DELETE FROM employee_projects WHERE project_id IN (
    SELECT id FROM projects WHERE end_date < '2023-01-01'
);

-- UPSERT：冲突时更新薪资
INSERT INTO employees (id, name, age, salary, dept_id, hire_date, email)
VALUES (1, '张三', 29, 16500, 1, '2020-03-15', 'zhangsan@example.com')
ON CONFLICT(id) DO UPDATE SET salary = excluded.salary, age = excluded.age;

-- INSERT INTO ... SELECT：从高薪视图复制到新表
CREATE TABLE high_salary_backup (
    emp_name TEXT, salary REAL, dept_name TEXT
);
INSERT INTO high_salary_backup SELECT * FROM v_high_salary_employees;
...
WITH
  ...
  above_avg AS (
    SELECT e.name, e.salary, e.dept_id, a.avg_salary
    FROM employees e JOIN avg_sal a ON e.dept_id = a.dept_id
    WHERE e.salary > a.avg_salary
  )
SELECT d.name AS 部门, aa.name AS 员工, aa.salary, ROUND(aa.avg_salary, 2) AS 部门均薪
FROM above_avg aa JOIN departments d ON aa.dept_id = d.id
...
SELECT n, n * n AS 平方, HEX(n) AS 十六进制 FROM seq;
...
INSERT INTO org_tree VALUES (1, '总公司', NULL);
INSERT INTO org_tree VALUES (2, '技术部', 1);
INSERT INTO org_tree VALUES (3, '前端组', 2);
INSERT INTO org_tree VALUES (4, '后端组', 2);
INSERT INTO org_tree VALUES (5, '市场部', 1);

WITH RECURSIVE org_path(id, name, path, level) AS (
    SELECT id, name, name, 0 FROM org_tree WHERE parent_id IS NULL
    UNION ALL
    SELECT t.id, t.name, op.path || ' > ' || t.name, op.level + 1
    FROM org_tree t JOIN org_path op ON t.parent_id = op.id
)
SELECT id, path AS 组织路径, level AS 层级 FROM org_path ORDER BY id;
...
SELECT emp_id FROM employee_projects WHERE project_id = 1
INTERSECT
SELECT emp_id FROM employee_projects WHERE project_id = 2;
...
SELECT e.name, d.name AS dept, e.salary,
    ...
    NTILE(2)     OVER (PARTITION BY e.dept_id ORDER BY e.salary DESC) AS ntile_2
FROM employees e
JOIN departments d ON e.dept_id = d.id;

-- LAG / LEAD：与前/后一名员工的薪资差
SELECT e.name, e.salary,
    LAG(e.salary, 1, 0)  OVER (ORDER BY e.salary DESC) AS prev_salary,
    LEAD(e.salary, 1, 0) OVER (ORDER BY e.salary DESC) AS next_salary,
    e.salary - LAG(e.salary, 1, 0) OVER (ORDER BY e.salary DESC) AS diff_prev
FROM employees e;

-- FIRST_VALUE / LAST_VALUE / NTH_VALUE
SELECT e.name, d.name AS dept, e.salary,
    FIRST_VALUE(e.name) OVER (PARTITION BY e.dept_id ORDER BY e.salary DESC) AS 最高薪者,
    LAST_VALUE(e.name)  OVER (PARTITION BY e.dept_id ORDER BY e.salary DESC
...
CREATE TABLE dept_budget (id INTEGER PRIMARY KEY, budget_limit REAL);
INSERT INTO dept_budget VALUES (1, 600000), (2, 400000), (3, 200000), (4, 300000);
SELECT * FROM departments NATURAL JOIN dept_budget;
...
FROM (SELECT e.name, e.salary, d.name AS dept FROM employees e JOIN departments d ON e.dept_id = d.id)
GROUP BY dept;
...
SELECT name FROM employees WHERE name GLOB '[周吴郑王]*';
...
SELECT
    date('now') AS 今天日期,
    time('now') AS 当前时间,
    datetime('now') AS 当前日期时间,
    datetime('now', '+1 day') AS 明天,
    datetime('now', '-7 days') AS 一周前,
    datetime('now', 'start of month') AS 月初,
    datetime('now', 'start of year') AS 年初,
    datetime('now', 'weekday 0') AS 本周日,
    strftime('%Y-%m-%d %H:%M:%S', 'now') AS 格式化,
    strftime('%w', 'now') AS 周几,
    strftime('%j', 'now') AS 年第几天,
    julianday('now') AS 儒略日,
    unixepoch('now') AS unix时间戳;
...
ALTER TABLE departments ADD COLUMN established_date TEXT;
